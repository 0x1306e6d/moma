...
// flash load "C:\Users\USER\Desktop\monday\week10\flashclear.axf"
// flash load "C:\Users\USER\Desktop\monday\week10\debug\week10.axf"
/* Private define ---------------------------------------------------------*/
#define LCD_CS (GPIOC_BASE + 0x0C)
#define LCD_RS (GPIOD_BASE + 0x0C)
/* Private variable ---------------------------------------------------------*/
uint16_t DeviceCode;
...
uint16_t LCD_ReadReg(uint16_t LCD_Reg) {
	...
	GPIO_ResetBits(GPIOC, GPIO_Pin_6);
	GPIO_SetBits(GPIOD, GPIO_Pin_13);
	GPIO_ResetBits(GPIOD, GPIO_Pin_15); //RD
...
void LCD_ShowNum(u8 x, u8 y, u32 num, u8 len, u16 PenColor, u16 BackColor) {
	...
	for (t = 0; t < len; t++) {
		...
		if (enshow == 0 && t < (len - 1)) {
			if (temp == 0) {
				LCD_ShowChar(x + (size / 2) * t, y, ' ', size, PenColor,
						BackColor);
				continue;
			} else
				enshow = 1;
		...
		LCD_ShowChar(x + (size / 2) * t, y, temp + '0', size, PenColor,
				BackColor);
	}
}
